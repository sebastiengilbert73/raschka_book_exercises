...
def main():
    logging.info("majority_vote_iris.main()")

    iris = datasets.load_iris()
    X, y = iris.data[50:, [1, 2]], iris.target[50:]  # We exclude the 50 1st lines, and
    # we keep only features 1 and 2
    le = LabelEncoder()
    ...
    for clf, label, clr, ls in zip(all_clf, clf_labels, colors, linestyles):
        ...
        plt.plot(fpr, tpr, color=clr, linestyle=ls, label=f'{label} (auc = {roc_auc:.4f})')
